...
def collate_fn(batch):
    ...
    data, text_data, speaker_id, target, len_data = list(), list(), list(), list(), list()
    for idx in range(len(batch)):
        ...
        # append target
        target.append(torch.tensor(batch[idx][3]))
...
def collate_fn_pt(batch):
    ...
    # max of 6s of data
    max_audio_len = min(max([b[0].shape[0] for b in batch]), 16000*6)
    data, speaker_id, target, len_data = list(), list(), list(), list()
    input_ids_list, attn_list =list(), list()
    
    for idx in range(len(batch)):
        data.append(padding_cropping(batch[idx][0], max_audio_len))
        input_ids, attention_mask = batch[idx][1]
    ...
    # input_ids_batch = pad_sequence(input_ids_list, batch_first=True, padding_value=tokenizer.pad_token_id)
    # attn_batch = pad_sequence(attn_list, batch_first=True, padding_value=0)   
    # print(f"dataloader: {data.shape, len_data.shape, target.shape}")
    return data, (input_ids_batch, attn_batch), speaker_id, target, len_data
...
class EmotionDatasetGenerator(Dataset):
    def __init__(
        ...
        """
        self.data_list              = data_list # lists of json entity. 
        ...
        if self.load_pt:
                if not hasattr(self, "pt_data"):  
                    self.pt_data = torch.load(self.data_list[0][4])
    ...
    def __getitem__(
        ...
        if self.data in ['iemocap', 'iemocap6']: 
            if self.load_pt:
                fname = self.data_list[item][0]  # Speaker ID (e.g., "Ses01F_impro01_F000")
                utt_dict = self.pt_data[fname]
                input_ids = utt_dict["input_ids"]
                attention_mask = utt_dict["attention_mask"]
                txt_data = (input_ids, attention_mask)
            else: 
                files = pd.read_csv(file_path)
                txt_data = files[files['Speaker'] == speaker_identifier]['Utterance'].values.tolist()
        elif self.data  in ['meld', 'meld7']:
            if self.load_pt:
                fname = self.data_list[item][0]
                utt_dict = self.pt_data[fname]   
                input_ids = utt_dict["input_ids"]
                attention_mask = utt_dict["attention_mask"]
                txt_data = (input_ids, attention_mask)
                ...
                txt_data = files[files['dia_utt'] == speaker_identifier]['Utterance'].values.tolist()
...
def map_label(
    ...
    label_dict = {
        ...
        "meld7": {"neutral": 0, "sadness": 1,  "anger": 2, "joy": 3, "surprise": 4, "fear":5, "disgust":6},
    }
    if dataset in ["iemocap", "iemocap6", "meld", "meld7", "crema_d", "iemocap_impro"]:
...
def load_pretrain_audios(
    ...
    for dataset in ['iemocap', 'iemocap6', 'meld', 'meld7', 'ravdess',  'cmu-mosei', ]:
...
def load_finetune_audios(
    ...
    if dataset in ["iemocap", "iemocap6"]:
        with open(str(Path(input_path).joinpath(f'iemocap6_fold{fold_idx}.json')), "r") as f: split_dict = json.load(f)
    elif dataset in ["meld", "meld7"]: ## add meld
...
def return_weights(
    ...
    if dataset in ["iemocap_impro"]:
        ...
        with open(str(Path(input_path).joinpath(f'crema_d_fold{fold_idx}.json')), "r") as f:
            split_dict = json.load(f)
    elif dataset in ["msp-podcast", "meld", "meld7"]:
        with open(str(Path(input_path).joinpath(f'{dataset}.json')), "r") as f:
            split_dict = json.load(f) 
    elif dataset in ["iemocap", "iemocap6"]:
        with open(str(Path(input_path).joinpath(f'iemocap6_fold{fold_idx}.json')), "r") as f:
            split_dict = json.load(f) 
    else:
    ...
    print(f"norm weights: {weights / weights.sum()}")
    print(f"log weights: { torch.log(weights)}")
    # exit()
    return weights
...
def set_finetune_dataloader(
    ...
    if is_distributed:
        ...
        if args.load_pt:
            collate_function = collate_fn_pt
        else: 
            collate_function = collate_fn
        
        if is_train:
            # class_counts = torch.tensor([4709, 683, 1109, 1743, 1205, 268, 271], dtype=torch.float32)
            # class_weights = 1.0 / (class_counts)
            # labels = torch.tensor([sample[-1] for sample in filtered_file_list], dtype=torch.long)
